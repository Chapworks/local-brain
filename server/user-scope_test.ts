...
Deno.test("userScope — user with id returns parameterized clause", () => {
  const result = userScope({ id: 42, name: "nick", username: "nick", isSuperuser: false, needsPasswordSetup: false }, 1);
  assertEquals(result.clause, "user_id = $1");
  assertEquals(result.params, [42]);
});

Deno.test("userScope — user with paramIdx 3", () => {
  const result = userScope({ id: 7, name: "test", username: "test", isSuperuser: false, needsPasswordSetup: false }, 3);
  assertEquals(result.clause, "user_id = $3");
  assertEquals(result.params, [7]);
});

Deno.test("userScope — user id is included in params", () => {
  const result = userScope({ id: 100, name: "admin", username: "admin", isSuperuser: true, needsPasswordSetup: false }, 2);
