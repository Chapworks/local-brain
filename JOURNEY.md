...
- **The context window tax.** Claudegram compacts context more frequently than Claude Code in the terminal. During the hardening session, compaction hit at 167.2K tokens — well before the 200K model limit. The reason: Claudegram's inputs are heavier per turn. Every round-trip carries system reminders (CLAUDE.md, git status, skill definitions, file-change notifications), Telegram formatting context, and the continuation summary from the previous compaction. When you read five full source files in parallel (which is efficient for coding), you burn 10-15K tokens in a single tool response. In the terminal, Claude Code prunes old tool results more aggressively and the per-turn overhead is lighter. The practical effect: a terminal session might go 30-40 turns before compressing; Claudegram hits it at 15-20 turns of substantive coding. This means each instruction from the user needs to be denser — you learn to batch requests ("implement all five, clean up the roadmap, commit and push") because each turn costs more. The irony isn't lost: the tool being used to build a persistent memory system is itself fighting memory limits. The compaction is automatic but lossy — you lose nuance, variable names get approximated, the AI has to re-read files it already read. Local Brain's approach (explicit, durable persistence) is the opposite of what's happening inside the tool building it. The constraint shaped the product.
